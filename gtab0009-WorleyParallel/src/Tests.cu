...
void test1() {
	...
	gpuErrchk( cudaFree(d_result_no_shared_mem) );
	gpuErrchk( cudaFree(d_result_shared_mem) );
...
void test2() {
	...
	// Correct output
	int correct_result[4][4] = {{0, 1, 2, 3},
								{1, 1, 2, 2},
								{2, 2, 1, 1},
								{3, 2, 1, 0}};
...
void test3() {
	// Testing larger dataset, with image 1000 x 1000 but compared with result from the serial implementation
...
void test4() {
	// Testing larger dataset, with image 2000 x 2000, with 2 points/tile. The result is compared with the result from the serial implementation
